...
The notebooks were developed primarily in a Kaggle environment.
...
and configure the input/output paths for the OHRC imagery.
...
to merge labeled data and prepare the YOLO dataset.
...
The notebook trains YOLO11s and produces:
...
Update the model and test-image paths in:
...
For the experimental depth-estimation pipeline, provide:

- an OHRC PNG image
- its corresponding PDS XML metadata
